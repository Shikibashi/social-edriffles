...
class FeedProviderSecurityContractTests(unittest.TestCase):
    def test_adversarial_matrix_is_complete_and_data_only(self):
        fixture = json.loads((ROOT / 'tests/fixtures/feed-provider-security.json').read_text())
        expected = {
            'huge-response', 'huge-compressed-response', 'excessive-candidates',
            'malicious-cursor', 'invalid-uri', 'invalid-cid', 'duplicate-candidate',
            'stale-batch', 'redirect-abuse', 'slow-provider', 'unavailable-provider',
            'forged-provider-identity', 'invalid-manifest-signature', 'revoked-key',
            'malformed-reason', 'hydration-mismatch', 'executable-payload',
