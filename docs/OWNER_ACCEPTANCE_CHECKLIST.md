...
| 2 | Open Home | The active feed, provider, algorithm/version, and provenance state are identifiable; an enabled local curation overlay is identified separately from the provider | Attention transparency | PASS | Live `https://social.edriffles.us/?cachebust=6b00d319` rendered `For You` with `Algorithm: Filtered For You + local curation (version content-filter/1 + local-curation/1)`, separate `AppView: Public Bluesky AppView (explicit read provider)`, provider DID `did:web:api.bsky.app`, feed owner DID/URI, objective, and device-local privacy scope. The local fixture remains separately documented below. |
...
| 27 | Open Services settings | PDS, AppView, feed provider, labeler, and resolver are shown as distinct actors when known | Polycentric services | PASS | Live `https://social.edriffles.us/settings/services?cachebust=6b00d319` rendered the account PDS `https://yellowfoot.us-west.host.bsky.network/` separately from `Public Bluesky AppView (explicit read provider)` (`did:web:api.bsky.app · https://api.bsky.app`). The current UI does not claim separate configured labeler/resolver providers; the feed card separately identifies its feed owner/provider. |
