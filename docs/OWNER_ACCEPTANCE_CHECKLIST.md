...
visibly attributed as `Public AT Protocol AppView`; it is not an implicit fallback
...
| 6 | Inspect the provenance card | Provider DID, feed owner when known, version/manifest status, objective, and privacy scope are not fabricated | Institutional anti-reification; attention transparency | PASS | Live card identifies `Public AT Protocol AppView (external read provider)`, DID `did:web:api.bsky.app`, the `for-you` feed owner/URI, `Manifest: unverified`, the actual local-curation objective, and device-local privacy scope. The disposable local fixture still identifies its own fixture AppView and feed owner when that launch command is used. |
...
| 64 | Inspect the browser title, logged-out navigation label, feed provenance, and Feed customization & data settings | The public product is branded `Edriffles`; the UI says `local curation`; it does not present the constitutional project name as a product slogan | Institutional anti-reification; product identity | RECHECK AFTER BRAND BUILD | The source now defaults to `Edriffles` and the web identity layer uses the Edriffles emblem/wordmark. Browser visual acceptance remains a separate owner check. Internal compatibility IDs remain intentionally unchanged. |
| 65 | Build the web artifact with the command in `docs/SOCIAL_EDRIFFLES_DEPLOYMENT.md` | The artifact is a static SPA whose title/metadata use `Edriffles`, whose user-facing origin is `https://social.edriffles.us`, whose deep routes load root-relative assets, and whose protocol providers remain separately configured | Polycentric services; meaningful exit | RECHECK AFTER BRAND BUILD | The production export must include Edriffles metadata, root-relative assets, browser icons, the Edriffles splash mark, accurate public-provider labeling, CSP allowances, complete Space URI validation, callback recovery, and Radlib OAuth scope enforcement. Browser password entry remains owner-controlled. |
...
- `Settings > Services` showed the account PDS
  ...
  `Public AT Protocol AppView (external read provider)` with DID
