...
| 2 | Open Home | The active feed, provider, algorithm/version, and provenance state are identifiable; an enabled local curation overlay is identified separately from the provider | Attention transparency | PASS | Local fixture walkthrough renders separate `Following` and `Discover` tabs with `Show feed details`; the card names the Project AppView, feed owner/provider, URI, manifest status, objective, and privacy scope. |
...
| 6 | Inspect the provenance card | Provider DID, feed owner when known, version/manifest status, objective, and privacy scope are not fabricated | Institutional anti-reification; attention transparency | PASS | The local card identifies Project AppView DID `did:plc:dw4kbjf5mn7nhenabiqpkyh3`, local feed owner `did:plc:tgdhu5j5on7rokehpgyzcben`, neutral feed URI ending `/social-discover`, provider DID `did:plc:3yyddqsbp64qbnelcev36sey`, and keeps manifest/objective/health limitations explicit. |
...
| 27 | Open Services settings | PDS, AppView, feed provider, labeler, and resolver are shown as distinct actors when known | Polycentric services | PASS | Live Services settings rendered the account PDS separately from the configured Project AppView; the feed provenance card separately identified the project feed provider. |
